...
async function crearNavegacion() {
    ...
    for (const browserType of ['chromium']) {
        ...
        let nombrePrueba = 'CrearNavegacion';
        ...
        for (var i=0; i<arrayUrls.length;i++) {
            ...
            await Promise.all([
                page.waitForNavigation(/*{ url: 'http://localhost:2369/ghost/#/settings/design' }*/),
            ...
            await page.press('text=You must specify a label Add >> [placeholder="Label"]', 'CapsLock');
            await page.press('text=You must specify a label Add >> [placeholder="Label"]', 'CapsLock');
            await page.fill('text=You must specify a label Add >> [placeholder="Label"]', 'Prueba');
            await page.click('button:has-text("Save")');
            await page.screenshot({ path:`${pathGeneral}${nombrePath}-pantalla${contador}-${browserType}.png` });
            contador++;

            await page.click('text=View site', {delay:5000});
...
(async () => console.log(await crearNavegacion()))();
