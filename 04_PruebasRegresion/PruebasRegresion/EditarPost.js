...
async function editarPost() {
    ...
    for (const browserType of ['chromium']) {
        ...
        for (var i=0; i<arrayUrls.length;i++) {
            ...
            await page.click('text=Welcome to Ghost By Ghost in Getting Started',{delay:1000});
            
            await page.screenshot({ path:`${pathGeneral}${nombrePath}-pantalla${contador}-${browserType}.png` });
            contador++;

            await page.click('textarea');
            await page.fill('textarea', 'Bienvenido a Ghost');
            await page.click('div[role="button"]:has-text("Update")');
            await page.click('button:has-text("Update")');
            
            await page.screenshot({ path:`${pathGeneral}${nombrePath}-pantalla${contador}-${browserType}.png` });
            contador++;

            await page.click('text=Posts' ,{delay:3000});
...
(async () => console.log(await editarPost()))();
