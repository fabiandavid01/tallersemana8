...
async function cambiarZonaH() {
    ...
    for (const browserType of ['chromium']) {
        ...
        for (var i=0; i<arrayUrls.length;i++) {
            ...
            await Promise.all([
                page.waitForNavigation(/*{ url: 'http://localhost:2369/ghost/#/settings/general' }*/),
            ...
            await page.click('.gh-setting .gh-setting-action .gh-btn');
            await page.click('main[role="main"] >> :nth-match(div:has-text("Site timezone Set the time and date of your publication, used for all published "), 3)',{delay:3000});

            await page.screenshot({ path:`${pathGeneral}${nombrePath}-pantalla${contador}-${browserType}.png` });
            contador++;

            await page.selectOption('select', 'America/Mexico_City');
            await page.click('button:has-text("Save settings")',{delay:3000});
...
(async () => console.log(await cambiarZonaH()))();
