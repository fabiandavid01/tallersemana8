...
async function eliminarColaborador() {
    ...
    for (const browserType of ['chromium']) {
        ...
        let nombrePrueba = 'EliminarColaborador'
        ...
        for (var i=0; i<arrayUrls.length;i++) {
            ...
            //Eliminar invitacion de colaborador

            await page.click('text=Staff');

            let correo = `fabiandavid${Math.floor(Math.random() * (30 - 3)) + 3}@gmail.com`
            await page.click('button:has-text("Invite people")');
            await page.fill('[placeholder="Email Address"]', correo);
            ...
            await page.click('text=View site');
            await page.click('text=Staff',{delay:3000});

            await page.screenshot({ path:`${pathGeneral}${nombrePath}-pantalla${contador}-${browserType}.png` });
            contador++;

            await page.click(`text= ${correo}`);
            await page.click('text=Revoke',{delay:3000});
...
(async () => console.log(await eliminarColaborador()))();
