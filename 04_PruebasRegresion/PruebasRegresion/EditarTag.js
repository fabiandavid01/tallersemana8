...
async function editarTag() {
    ...
    for (const browserType of ['chromium']) {
        ...
        for (var i=0; i<arrayUrls.length;i++) {
            ...
            await Promise.all([
                page.waitForNavigation(/*{ url: 'http://localhost:2369/ghost/#/tags' }*/),
                page.click('text=Tags')
              ]);

            await page.click('text=New tag');

            let nombreEtiqueta = `etiqueta${Math.floor(Math.random() * (30 - 3)) + 3}`
            await page.fill('input[name="name"]', nombreEtiqueta);

            await Promise.all([
                page.waitForNavigation(/*{ url: 'http://localhost:2369/ghost/#/tags/perrito' }*/),
                page.click('button:has-text("Save")')
            ]);

            await page.click('text=Tags', {delay:3000});

            await page.screenshot({ path:`${pathGeneral}${nombrePath}-pantalla${contador}-${browserType}.png` });
            contador++;

            let etiqueta = `a:has-text("${nombreEtiqueta}")`

            await page.click(etiqueta);
            await page.fill('input[name="name"]', 'pruebaRenombrada');
            await page.click('button:has-text("Save")',{delay:3000});
...
(async () => console.log(await editarTag()))();
