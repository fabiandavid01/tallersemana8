//Configurar enlace a cuenta erronea de Facebook
describe('Prueba la pagina ghost', () => {
    ...
    it('Enlazar con facebook', () => {
        ...
        cy.get('input.ember-text-field.gh-input.ember-view').eq(0).type('https://web.google.com/aqp.pyladies')

        cy.get('button.gh-btn.gh-btn-blue.gh-btn-icon.ember-view').click()

        cy.get('div.form-group.error.ember-view').within(() => {
            cy.get('p').then(($item) => {
                expect($item[0].innerText).to.equal('The URL must be in a format like https://www.facebook.com/yourPage')
            })
        })
